...
int receive_ping_reply(int sock) {
  ...
  icmp_len = len - ip_header_len;
    ...
    printf("send = %ld, rec = %ld\n", tvsend->tv_sec, tvrecv.tv_sec);

    rtt = tvrecv.tv_sec * 1000+tvrecv.tv_usec / 1000;

    printf("%zd bytes from %s icmp_seq=%u ttl=%d rtt=%.3f\n",
            len, inet_ntoa(rcv_addr.sin_addr), icmp->icmp_seq, ip->ip_ttl, rtt);
